...
function stubSpawn(overrides: Partial<Record<keyof ChronicleWorkerApi, unknown>> = {}) {
  ...
  const spawn: WorkerSpawn = () => {
    ...
    const api = {
      matcherVersion: () => Promise.resolve("stub"),
...
describe("WorkerPool", () => {
  ...
  it("marks a faulted slot dead, keeps serving from live slots, and fails only when all are dead", async () => {
    ...
    await expect(pool.submit(async (api) => api.matcherVersion())).resolves.toBe("stub");
    faults[1].reject(new Error("slot 1 died"));
    await Promise.resolve();
    await expect(pool.submit(async (api) => api.matcherVersion())).rejects.toThrow(
  ...
  it("rejects queued waiters when the last live slot dies mid-wait", async () => {
    ...
    const waiting = pool.submit(async (api) => api.matcherVersion());
...
describe("shared worker fault handling (fake Worker global)", () => {
  it("rejects loudly on a worker error event, evicts the singleton, and re-spawns on retry", async () => {
    ...
    const pending = getMatcherVersion();
  ...
  it("warmMatcher swallows the failure (best-effort warmup)", async () => {
    const pending = warmMatcher();
