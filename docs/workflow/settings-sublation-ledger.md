...
glossaries in the [prior-art vocabulary](prior-art-vocabulary.md) and
[sublation audit](sublation-audit.md).

Legend for the "maps to" column: node ids from the
[prior-art vocabulary](prior-art-vocabulary.md); *strategy* = named reconstruction
algorithm ([device-state model](device-state-machine.md), change #2); *rule* = ordered
app-policy/interval-quality rule row;
...
| `proximity_interval_seconds` | tune on `chronicle_lifecycle_matcher` (intra-app teardown grace — NOT EYES proximity; see the [sublation audit](sublation-audit.md)) |
...
| — no-witness day min-app rule | `effective_usage` policy conditioned on `screen_witness_coverage` (recorded as policy + evidence basis, per the [device-state model](device-state-machine.md)) |
...
| Missing-day accounting + coverage invariant | `day_coverage` (invariant = hard error; see [workflow graph design](design-draft.md#section-6--error-handling-draft)) |

## C. Prior-art tools

This section covers the settings level; the concept level is in the
[prior-art vocabulary](prior-art-vocabulary.md) and [sublation audit](sublation-audit.md).
...
| P&T totals/IDs/sequences/repertoires/trajectories | `device_usage` + named output contracts ([device-state model](device-state-machine.md), change #12) |
...
| EYES SHUTDOWN>IDLE>GAP precedence | fusion precedence table in the [multistream model](multistream-superset.md) |
...
element beyond the expansions already adopted in the [sublation audit](sublation-audit.md)
and [device-state model](device-state-machine.md) — and two settings are direct
...
fixture conformance (see the [sublation audit](sublation-audit.md)) — the mapping above is
the semantic account, not the parity
proof.
