...
> findings to the current query-registry Rust/Salsa runtime, not to the old paths.
...
### V3. Covering-array sampling of the registered configuration space
The 2^12 bypass enumeration is exhaustive over gates, but nothing guarantees interaction
coverage across all registered settings (the sentinel bug was an interaction: filter ON × walk
...
all registered knobs: which are exercised by production (locked defaults), which by tests only,
...
`_tools/derive.py` patterns applied to the contract-derived query graph and settings: articulation points
...
| Config-space coverage | Adopt PICT (or CAgen) | PICT: single binary, constraints, n-way; maintained-enough (v3.7.4) | generated model over registered settings + constraint list |
...
contract-derived graph. XState `xstate/graph` path enumeration optional second axis (note:
...
vitest, `pypict`/subprocess in pytest, ONE SSOT model file for the registered keys. **CAgen**
...
registration) computes the t-way coverage of an EXISTING test set. Express the registered-knob
