#!/usr/bin/env bash
# check-licenses.sh
# License compliance check across Python, npm, and Rust dependencies.
#
...
bad_python=$(
    "$REPO_ROOT/.venv/bin/pip-licenses" \
        --format=csv \
        --order=license \
        --ignore-packages chronicle-android-preprocessor \
        2>&1 \
    | grep -iE "AGPL|Proprietary|Commercial" || true
)
if [ -n "$bad_python" ]; then
    echo "$bad_python"
    echo "ERROR: Incompatible Python licenses found above" >&2
    FAIL=1
else
