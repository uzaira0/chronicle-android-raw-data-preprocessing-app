...
and every one of them still matches at least one — the absolute figure moved
because the crate's mutant set is not the sweep's any more, so only the
...
mutant and **no** mutant the suite kills. Five carry a line number, because the
...
gate. All five were re-pinned when the two encode helpers below were split,
which moved every line after `encode_reconstruction_base` down by 21.
