...
the [device-state model](device-state-machine.md) and amend this file; amendments are
marked ✎. All spot-checkable second-audit
...
|---|---|
| P&T forward-pairing episodes | ✎ REVISED: NOT expressible as a closer-vocabulary preset (verified). Start-only forward-pairing changes episode counts, dedups consecutive resumes, and takes endpoints from the merged event stream. Requires `reconstruct_episodes` strategy = `parry_toth_forward_pair_2025` (first-class versioned algorithm; [device-state model](device-state-machine.md), change #2) |
...
| Culverhouse long-event actions | ✎ REVISED: interval-quality policy applying to App Usage, Session AND Glance rows (`apply_long_event_action_to = "all"`), default action = truncate to the 10-min bad-app cap, long_3h/long_6h flags mutually exclusive → a general interval-quality-policy node, not a knob on `effective_usage` alone ([device-state model](device-state-machine.md), change #5) |
| Culverhouse timezone modal filter | native timezone-handling option |

## The expansions

The first audit's five were amended by the second audit; full detail is in the
...
4. **Provenance → immutable lineage ledger** — appended flags (`end_source`, `credit_state`,
   ...
   the Rust runtime records every physical query state, content identity, and reason in
   the run manifest and projects it into `chron:QueryExecution` activities in the PROV-O
   sidecar (SHACL-validated by `make -C web/schema check`). The root run is a
   `chron:OperationExecution`; mapped semantic operations remain prospective because a
   fused query completing cannot prove that every mapped operation was applied. Per-ROW lineage
...
Plus three new expansion classes the first audit missed (see changes #2, #3, #7, and #8
in the [device-state model](device-state-machine.md)):
