 ...
 *
 * Invariant: a file that already finished (status `"complete"` or `"error"`) is
 * never reverted to a non-terminal status by a later event. In the worker path
 * the Comlink-proxied `onProgress` callback travels a *separate* MessagePort
 * from the RPC return value, so a trailing `step` event (e.g. "output" 100%)
 * can be delivered *after* `file-complete`. Without this guard that late event
 * would leave the row stuck at "Building output 100%" even though processing
 * already finished.
 */
export function applyProgressEvent(
  ...
  const existingTerminal = existing.status === "complete" || existing.status === "error";
