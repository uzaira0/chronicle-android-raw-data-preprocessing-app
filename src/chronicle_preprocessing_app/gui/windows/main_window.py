...
from chronicle_preprocessing_app.gui.panels.config_panel import ConfigPanel
from chronicle_preprocessing_app.gui.panels.options_panel import OptionsPanel
from chronicle_preprocessing_app.gui.panels.plotting_panel import PlottingPanel
from chronicle_preprocessing_app.gui.panels.status_panel import StatusPanel
from chronicle_preprocessing_app.gui.workers.preprocessing_thread import PreprocessingThread
...
class ChronicleAndroidRawDataPreprocessingGUI(QMainWindow):
    ...
    def start_preprocessing(self) -> None:
        ...
        if not self.options.enable_preprocessing and not self.options.enable_plotting:
            QMessageBox.warning(
                self,
                "Warning",
                "Please select at least one operation (Preprocess or Plot)",
            )
            return
    ...
    def _load_config_to_options(self, config: dict) -> None:
        ...
        if "allow_stop_event_reuse" in config:
    ...
    def _update_ui_from_options(self) -> None:
        ...
        # Update algorithm settings in options panel
        if self.options.allow_stop_event_reuse:
            self.options_panel.allow_stop_reuse_radio.setChecked(True)
        else:
