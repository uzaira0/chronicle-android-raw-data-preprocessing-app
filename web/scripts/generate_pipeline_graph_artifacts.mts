...
const INTERACTION_TYPES_OUTPUT_PATH = path.resolve(
  HERE,
...
async function buildProjection(rustContract: RustPipelineContract): Promise<string> {
...
function buildInteractionTypes(rustContract: RustPipelineContract): string {
  const names = rustContract.canonicalInteractionTypes;
  if (names.length === 0 || new Set(names).size !== names.length) {
    throw new Error("Rust contract must declare a non-empty, duplicate-free canonical interaction-type list");
  }
  const sorted = [...names].sort((left, right) => left.localeCompare(right));
  if (sorted.some((name, index) => name !== names[index])) {
    throw new Error("Rust canonical interaction types must be sorted for deterministic UI generation");
  }
  return [
    ...
    `export const PREPROCESSOR_VERSION = ${JSON.stringify(rustContract.preprocessorVersion)};`,
...
async function main(): Promise<void> {
  ...
  const projection = await buildProjection(rustContract);
  const interactionTypes = buildInteractionTypes(rustContract);
  if (check) {
    const existing = await readFile(OUTPUT_PATH, "utf-8").catch(() => null);
    const existingInteractionTypes = await readFile(INTERACTION_TYPES_OUTPUT_PATH, "utf-8").catch(
      () => null,
    );
    if (existing !== projection || existingInteractionTypes !== interactionTypes) {
      console.error(
        `DRIFT: generated graph/UI artifacts do not match the Rust step contract — ` +
    ...
    console.log(JSON.stringify({
      ...
      mode: "check",
      artifacts: [
        "schema/chronicle-pipeline-graph.yaml",
        "src/lib/generatedInteractionTypes.ts",
      ],
    }));
    return;
