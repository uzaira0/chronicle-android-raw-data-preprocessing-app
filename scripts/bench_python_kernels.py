...
def polars_format(ts_ns: list[int], tz: str) -> list[str]:
    ...
    df = df.with_columns(
        pl.col("ts").dt.convert_time_zone(tz).dt.strftime("%Y-%m-%d %H:%M:%S%z").alias("ts_str")
    )
...
def polars_sort(ts_ns: list[int]) -> list[int]:
    df = pl.DataFrame(
        {
            "ts": pl.Series("ts", ts_ns, dtype=pl.Int64),
            "i": pl.arange(0, len(ts_ns), eager=True).cast(pl.UInt32),
        }
    )
...
def bench_fixture(path: Path) -> FixtureBench:
    ...
    parity_fmt = len(polars_strs) == len(pyo3_strs) and all(
        ...
        for a, b in zip(polars_strs, pyo3_strs, strict=False)
    )
...
def main() -> int:
    ...
    for name in FIXTURES:
        ...
        emit(
            fmt_summary_line(
                "sort_by_timestamp_stable", r.sort_polars, r.sort_pyo3, r.parity_sort_match
            )
        )
    ...
    if results:
        ...
        for op_name, polars_attr, pyo3_attr, parity_attr in [
            ...
            emit(
                f"  {op_name:<28} polars={fmt_ms(p_med)}  pyo3={fmt_ms(r_med)}  speedup={speedup:5.2f}x  parity={parity_str}"
            )
