...
> authority is the query-registry Rust/Salsa runtime and this repository no longer
...
|---|---|---|
| **Screen-gated valid-usage credit** | Rewrites each App-Usage session into screen-ON ∩ device-alive intervals, 6 h truncate (see [artifact and port semantics](artifact-port-semantics.md)) | `s14_credit.py` |
...
The features to port client-side are exactly: screen-gated credit, kids relabel,
study-window filter,
