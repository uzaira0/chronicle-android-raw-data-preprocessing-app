...
| Run all existing empirical campaigns on physical events | done | Configuration, artifact, timestamp-boundary, interaction, mixed-role, and semantic-model mutation campaigns regenerated the current dependency certificate. All six ledgers under `web/src/lib/pipelineGraph/golden/family-expected/` and `.semantic-federation/proofs/dependency-certificate.json` were last written by `make dependency-evidence` in `3c598ee`; the runtime suite passes 60/60. |
