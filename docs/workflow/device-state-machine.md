...
platform semantics (AOSP source), and the earlier documents in this workflow knowledge
base. Every cheaply
...
GAP ≈ `observation=silent` overlay; the [multistream gap taxonomy](multistream-superset.md)
= the `gap_cause` overlay.
...
- Silence: below the liveness threshold ⇒ `observation=silent`, physical belief unchanged
  ...
  multi-stream witnesses described by the [multistream model](multistream-superset.md).
...
10. Named-preset sublation claim withdrawn until fixture conformance exists (see the
    [sublation audit](sublation-audit.md)).
11. Prior-art catalog corrected (see the [prior-art vocabulary](prior-art-vocabulary.md)
    and [sublation audit](sublation-audit.md)).
...
sentences. See the [typed-edge ontology](typed-edge-ontology.md) for the query set and
rendering rules.
