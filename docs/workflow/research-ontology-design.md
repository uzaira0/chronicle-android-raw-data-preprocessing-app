...
Turn the workflow contract (semantic operations, typed artifacts, configuration
dependencies, applicability, memoized recompute, and byte-exact parity) plus the
...
- **Runtime lineage projected into the sidecar — DONE (2026-07, execution-lineage wave).**
  The run-level `prov:Activity` sidecar carries operation lineage: every root or nested
  execution is a `chron:OperationExecution` activity with `chron:executes_operation` →
  minted typed `chron:OperationDefinition` resources (`chron:part_of_operation` recursion),
  ...
  execution ledger — physical query records are assembled once and the manifest, sidecar,
  and Pipeline Explorer metrics are projections of that single ledger. A7
  (`chron:OperationExecutionContractShape`) extends the hand-authored
...
- **Still deferred (documented, NOT invented):** BFO-vs-DOLCE upper grounding — federate later via
  an SSSOM mapping set; do not re-ground this module (this document, D2).

## Inputs taken into account (every one)

| Input | What it contributes | Source |
|---|---|---|
| **P&T (Parry & Toth 2025)** | glances/sessions/episodes taxonomy; forward-pairing as a named strategy | [prior-art vocabulary](prior-art-vocabulary.md); OSF mfnu9 |
| **EYES** (ACOI-UofSC) | complement device-state model (SHUTDOWN/IDLE/GAP/GLANCE), FAU=App∩ACTIVE, mass-based | [prior-art vocabulary](prior-art-vocabulary.md); ported `device_states.py` |
| **Culverhouse** | downstream trim-and-log; bad-app caps; day flags | [prior-art vocabulary](prior-art-vocabulary.md); ported `cleaning.py` |
| **Our engine** | Registered operations and queries, configuration dependencies, absence semantics, person layer, integrity parameters | `chronicle-workflow.yaml`, `workflow_contract.rs` |
...
| **CAFE / DREAMER** | the study/measurement context this instrument serves | [prior-art vocabulary](prior-art-vocabulary.md) |
...
- `WorkflowPlan` ≡ `p-plan:Plan` (itself a `prov:Plan` in P-Plan);
  `OperationDefinition` maps broadly to `p-plan:Step` while remaining count-neutral and
  recursively composable (id, verb, engine, consumes, produces, depends_on, phase, fatal).
- `OperationExecution` ≡ `prov:Activity` with `used`/`generated` edges.
...
- This restores (does not regress) the factored
  [device-state model](device-state-machine.md): a gap is an
...
Model provenance with **PROV-O + P-Plan**: the **`WorkflowPlan` is the prospective
`p-plan:Plan`** (operations map broadly to `p-plan:Step`, parameters = variables/bindings); the `ParameterSet`
...
2. Separate WorkflowPlan / OperationDefinition / Procedure / Execution / ParameterSet /
   ParameterBinding.
...
8. Restore the factored [device-state model](device-state-machine.md); forbid gap/shutdown →
   physical-state assertions.
9. Reconcile graph-node and configuration-key registries from their sources of truth + add generated-artifact
   drift checks.
