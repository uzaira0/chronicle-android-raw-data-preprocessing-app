...
import type { BrowserProcessingOptions, MatcherInput, MatcherOutput, SplitterInput, SplitterOutput } from "@/lib/types";
...
const JUNK_PACKAGE = "com.junk.game";
const BG_PACKAGE = "com.bg.music";
...
function buildSupport(options: BrowserProcessingOptions): PipelineSupportData {
  ...
  return {
    ...
    codebookMap: options.useAppCodebook
      ? new Map([
          [
            "com.valid.chat",
            { genre: "Communication", store: "Play Store", storeUrl: "", iconUrl: "" } as never,
          ],
        ])
      : new Map(),
...
const RUN_KEYS = (options: BrowserProcessingOptions, extra?: Partial<{ supportFileHashes: Record<string, string>; inputHash: string }>) => ({
...
const ALL_ON: BrowserProcessingOptions = {
...
function descendantsOf(seed: ReadonlySet<string>): Set<string> {
...
function expectedBypassed(id: string, o: BrowserProcessingOptions): boolean {
  switch (id) {
    case "parse_events":
...
const SUPPORT_FIELD_TO_FILE: Record<keyof PipelineSupportData, string> = {
...
const VALID_SUPPORT_FILE_KEYS = new Set(Object.values(SUPPORT_FIELD_TO_FILE));
...
describe("5. Incremental recompute (dirty cone per option key)", () => {
  /** A value guaranteed to change the key, valid for the pipeline. */
  function altValue(key: string, value: unknown): unknown {
